...
%size(ky)=[1,50] ; size(kx)=[50,1]; size(use_sz)=50,50
%[Y,I]=max(M,[],1), 在第1维方向上取最大值，也就是每列最大值，结果存在Y里，I里存的是每列最大值的行位置
...
max_row_perm = permute(max_row, [2 3 1]);% 50x5
col = max_col(:)';%col坐标
row = max_row_perm(  sub2ind(size(max_row_perm), col, 1:size(response,3) )  );%找到列坐标
%相对位移 mod（a，b）求得是a/b 的
...
% pre-compute complex exponential     unit virtua = 1i=‘i’
